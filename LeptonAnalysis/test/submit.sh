...
SOURCEDIR=/afs/cern.ch/user/j/jhgoh/public/sources/CMG/V5_10_0
for DATASET in $SOURCEDIR/*.txt; do
  SAMPLE=`basename $DATASET | sed s/.txt//g`
  #SAMPLE=`echo $DATASET | cut -d- -f1`
  #PROD=`echo $DATASET | cut -d- -f2`
  #SOURCE="$CMSSW_BASE/src/KoPFA/CommonTools/python/Sources/CMG/V5_10_0/${PROD}/cmgTuple_${SAMPLE}_cff.py"
  SOURCE="$SOURCEDIR/$SAMPLE.txt"
  ...
  for SECTION in `seq 0 $NSECTION`; do
    bsub -oo log/${SAMPLE}_${SECTION}.log -q 8nh run.sh $SAMPLE $SECTION $(($NSECTION+1))
  done
done
